...
impl NetworkLayer {
    pub fn new(node_count: usize, previous_node_count: usize) -> NetworkLayer {
    ...
    pub fn new(
        ...
        let layers = (0..(hidden_layer_count + 1))
            .map(|layer_index| {
                ...
                NetworkLayer::new(node_count, previous_node_count)
            })
            .collect();
...
mod test {
    ...
    fn network_layer() {
        ...
        assert_eq!(
            layer.node_weights.len(),
            3,
            "There were three node weights created"
        );
        ...
        assert_eq!(
            layer.node_biases.len(),
            3,
            "There were three node biases created"
        );
    }
